	...
	public function addProduct($products)
	{
		foreach ($products as $product) {
            ...
            if(!empty($product["DETAIL_PICTURE_URL"])) {
                $detailImg = \CFile::MakeFileArray($product["DETAIL_PICTURE_URL"]);
                $imageId = \CFile::SaveFile(
                    $detailImg,
                    '/iblock',
                    false,
                ...
                $arFields["DETAIL_PICTURE"] = $detailImg;
                $arFields["PREVIEW_PICTURE"] = \CFile::MakeFileArray($previewImg["src"]);
                $arFieldsItem["DETAIL_PICTURE"] = $arFields["DETAIL_PICTURE"];
                $arFieldsItem["PREVIEW_PICTURE"] = $arFields["PREVIEW_PICTURE"];
            } else {
                $arFieldsItem["DETAIL_PICTURE"] = array('del' => 'Y');
			...
			if ($product["PARENT_GROUP"] != '' && $itemId = $this->getItemIdByProperty("IIKO_ID", $product["PARENT_GROUP"])) {
                ...
                if (!empty($arFieldsItem)) {
                    $this->updateIblockElement($itemId, $arFieldsItem);
                }
